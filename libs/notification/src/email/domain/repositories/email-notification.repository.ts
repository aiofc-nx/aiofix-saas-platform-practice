/**
 * @file email-notification.repository.ts
 * @description 邮件通知仓储接口
 *
 * 该接口定义邮件通知数据访问的契约，隔离领域层和基础设施层。
 * 遵循DDD原则，保持领域逻辑的纯粹性。
 */

import { EmailNotification } from '../entities/email-notification.entity';
import { Uuid } from '@aiofix/shared';
import { NotificationStatus, NotificationPriority } from '@aiofix/shared';

/**
 * @interface EmailNotificationRepository
 * @description 邮件通知仓储接口
 *
 * 主要原理与机制：
 * 1. 定义邮件通知数据访问的契约
 * 2. 隔离领域层和基础设施层
 * 3. 支持不同的存储实现
 * 4. 保持领域逻辑的纯粹性
 *
 * 功能与业务规则：
 * 1. 基础CRUD操作
 * 2. 业务查询方法
 * 3. 事务管理支持
 * 4. 并发控制支持
 */
export interface EmailNotificationRepository {
  /**
   * @method save
   * @description 保存邮件通知
   */
  save(notification: EmailNotification): Promise<void>;

  /**
   * @method findById
   * @description 根据ID查找邮件通知
   */
  findById(id: Uuid): Promise<EmailNotification | null>;

  /**
   * @method findByTenant
   * @description 查找租户下的邮件通知
   */
  findByTenant(
    tenantId: string,
    limit?: number,
    offset?: number
  ): Promise<EmailNotification[]>;

  /**
   * @method findByStatus
   * @description 根据状态查找邮件通知
   */
  findByStatus(
    status: NotificationStatus,
    tenantId?: string,
    limit?: number,
    offset?: number
  ): Promise<EmailNotification[]>;

  /**
   * @method findByPriority
   * @description 根据优先级查找邮件通知
   */
  findByPriority(
    priority: NotificationPriority,
    tenantId?: string,
    limit?: number,
    offset?: number
  ): Promise<EmailNotification[]>;

  /**
   * @method findByTemplate
   * @description 根据模板查找邮件通知
   */
  findByTemplate(
    templateId: string,
    tenantId?: string,
    limit?: number,
    offset?: number
  ): Promise<EmailNotification[]>;

  /**
   * @method findByRecipient
   * @description 根据收件人查找邮件通知
   */
  findByRecipient(
    recipient: string,
    tenantId?: string,
    limit?: number,
    offset?: number
  ): Promise<EmailNotification[]>;

  /**
   * @method findPendingNotifications
   * @description 查找待发送的通知
   */
  findPendingNotifications(
    tenantId?: string,
    limit?: number,
    offset?: number
  ): Promise<EmailNotification[]>;

  /**
   * @method findScheduledNotifications
   * @description 查找定时发送的通知
   */
  findScheduledNotifications(
    fromDate: Date,
    toDate: Date,
    tenantId?: string,
    limit?: number,
    offset?: number
  ): Promise<EmailNotification[]>;

  /**
   * @method findFailedNotifications
   * @description 查找发送失败的通知
   */
  findFailedNotifications(
    tenantId?: string,
    limit?: number,
    offset?: number
  ): Promise<EmailNotification[]>;

  /**
   * @method delete
   * @description 删除邮件通知
   */
  delete(id: Uuid): Promise<void>;

  /**
   * @method exists
   * @description 检查邮件通知是否存在
   */
  exists(id: Uuid): Promise<boolean>;

  /**
   * @method count
   * @description 统计邮件通知数量
   */
  count(tenantId?: string, status?: NotificationStatus): Promise<number>;

  /**
   * @method countByStatus
   * @description 按状态统计邮件通知数量
   */
  countByStatus(tenantId?: string): Promise<Record<NotificationStatus, number>>;

  /**
   * @method countByPriority
   * @description 按优先级统计邮件通知数量
   */
  countByPriority(
    tenantId?: string
  ): Promise<Record<NotificationPriority, number>>;

  /**
   * @method getStatistics
   * @description 获取邮件通知统计信息
   */
  getStatistics(
    tenantId?: string,
    fromDate?: Date,
    toDate?: Date
  ): Promise<{
    total: number;
    sent: number;
    failed: number;
    pending: number;
    successRate: number;
    averageDeliveryTime: number;
  }>;
}
