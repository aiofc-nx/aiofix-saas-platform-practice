...
export class ActivateOrganizationUseCase {
   ...
   * @returns 激活组织响应
   */
  async execute(request: ActivateOrganizationRequest): Promise<BaseResponse> {
    try {
      this.logger.info('开始激活组织', LogContext.BUSINESS, {
      ...
      if (organization.status === OrganizationStatus.ACTIVE) {
        this.logger.warn('组织已经是激活状态', LogContext.BUSINESS, {
          organizationId: request.organizationId,
        });
        return {
          success: true,
      ...
      // 5. 激活组织
      organizationAggregate.activate(request.currentUserId);
      ...
      this.logger.info('组织激活成功', LogContext.BUSINESS, {
        organizationId: request.organizationId,
        currentUserId: request.currentUserId,
      });
      ...
      this.logger.error('组织激活失败', LogContext.BUSINESS, {
        organizationId: request.organizationId,
        currentUserId: request.currentUserId,
   ...
   * @param request 激活组织请求
   */
  private validateRequest(request: ActivateOrganizationRequest): void {
