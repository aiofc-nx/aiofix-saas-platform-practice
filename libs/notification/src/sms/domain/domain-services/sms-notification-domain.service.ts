/**
 * @file sms-notification-domain.service.ts
 * @description 短信通知领域服务
 *
 * 该服务处理短信通知的复杂业务逻辑，包括验证、路由、重试等。
 * 遵循DDD原则，保持领域逻辑的纯粹性。
 */
...
/**
 * @interface SmsRoutingResult
 * @description 短信路由结果接口
 */
export interface SmsRoutingResult {
  shouldSend: boolean;
  reason?: string;
...
/**
 * @class SmsNotificationDomainService
 * @description 短信通知领域服务
 *
 * 主要原理与机制：
 * 1. 处理跨聚合的业务逻辑
...
export class SmsNotificationDomainService {
  ...
  /**
   * @method validateSmsNotification
   * @description 验证短信通知
   */
  validateSmsNotification(notification: SmsNotification): SmsValidationResult {
    const errors: string[] = [];
    const warnings: string[] = [];

    // 1. 验证收件人
    if (notification.recipients.length === 0) {
      errors.push('短信通知必须包含至少一个收件人');
    }

    // 验证每个收件人的手机号格式
    for (const recipient of notification.recipients) {
      try {
        PhoneNumber.create(recipient.value);
      } catch {
        errors.push(`无效的手机号码: ${recipient.value}`);
      }
    }

    // 2. 验证模板ID
    if (!notification.templateId) {
      errors.push('短信通知必须包含有效的模板ID');
    }

    // 3. 验证优先级
    if (!Object.values(NotificationPriority).includes(notification.priority)) {
      errors.push('短信通知必须包含有效的优先级');
    }
    ...
    if (dataSize > 1000) {
      warnings.push('短信数据过大，可能影响发送性能');
    }

    // 6. 验证收件人数量
    if (notification.recipients.length > 50) {
  ...
  /**
   * @method determineSmsRouting
   * @description 确定短信路由策略
   */
  determineSmsRouting(
  ...
  async optimizeBatchSending(
    ...
    const normalPriority: SmsNotification[] = [];
    const lowPriority: SmsNotification[] = [];
    ...
    // 2. 按收件人分组（避免重复发送）
    const recipientGroups = new Map<string, SmsNotification[]>();
    for (const notification of notifications) {
      for (const recipient of notification.recipients) {
    ...
    // 3. 生成批量发送计划
    const batches: SmsNotification[][] = [];
    let currentBatch: SmsNotification[] = [];
   ...
   */
  private shouldRateLimit(_notification: SmsNotification): boolean {
