/**
 * @file webhook-notification-domain.service.ts
 * @description Webhook通知领域服务
 *
 * 该服务处理Webhook通知的复杂业务逻辑，包括验证、路由、重试等。
 * 遵循DDD原则，保持领域逻辑的纯粹性。
 */
...
/**
 * @class WebhookNotificationDomainService
 * @description Webhook通知领域服务
 *
 * 主要原理与机制：
 * 1. 处理跨聚合的业务逻辑
...
export class WebhookNotificationDomainService {
  ...
  /**
   * @method validateWebhookNotification
  ...
  validateWebhookNotification(
    ...
    if (notification.recipients.length === 0) {
      errors.push('Webhook通知必须包含至少一个收件人');
    }

    // 验证每个收件人的URL格式
    for (const recipient of notification.recipients) {
      if (!this.isValidWebhookUrl(recipient)) {
        errors.push(`无效的Webhook URL: ${recipient}`);
      }
    }

    // 2. 验证模板ID
    if (!notification.templateId) {
      errors.push('Webhook通知必须包含有效的模板ID');
    }

    // 3. 验证优先级
    if (!Object.values(NotificationPriority).includes(notification.priority)) {
      errors.push('Webhook通知必须包含有效的优先级');
    }
    ...
    if (dataSize > 100000) {
      warnings.push('Webhook数据过大，可能影响发送性能');
    }

    // 6. 验证收件人数量
    if (notification.recipients.length > 10) {
  ...
  /**
   * @method determineWebhookRouting
   * @description 确定Webhook路由策略
   */
  ...
  async optimizeBatchSending(
    ...
    for (const notification of notifications) {
      ...
      if (hasHttps) {
        ...
        httpNotifications.push(notification);
      }
    }

    // 2. 按优先级分组
    const highPriority: WebhookNotification[] = [];
    const normalPriority: WebhookNotification[] = [];
    const lowPriority: WebhookNotification[] = [];
    ...
    // 3. 生成批量发送计划
    const batches: WebhookNotification[][] = [];
    let currentBatch: WebhookNotification[] = [];
  ...
  /**
   * @method getWebhookStatistics
   * @description 获取Webhook统计信息
   */
   ...
   */
  private shouldRateLimit(_notification: WebhookNotification): boolean {
