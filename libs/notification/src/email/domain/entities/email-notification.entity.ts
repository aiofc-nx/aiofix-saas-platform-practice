/**
 * @file email-notification.entity.ts
 * @description 邮件通知实体
 *
 * 该实体负责管理邮件通知的核心业务逻辑，包括：
 * - 邮件通知的创建和状态管理
 * - 邮件发送的验证和业务规则
 * - 邮件通知的生命周期管理
 *
...
{
  private readonly _type: NotificationType = NotificationType.EMAIL;
  private readonly _templateId: Uuid;
  private readonly _recipients: EmailAddress[];
  private readonly _subject?: EmailSubject;
   ...
   * @description 获取收件人列表
   * @returns {EmailAddress[]} 收件人列表
   */
  get recipients(): EmailAddress[] {
    return [...this._recipients] as EmailAddress[];
  }

  /**
   * @method getRecipientEmails
   * @description 获取收件人邮箱地址字符串列表
   * @returns {string[]} 收件人邮箱地址列表
   */
  get recipientEmails(): string[] {
    return this._recipients.map((recipient: EmailAddress) => recipient.value);
  }

  /**
   * @method getSubject
   * @description 获取邮件主题
   * @returns {EmailSubject | undefined} 邮件主题
   */
  get subject(): EmailSubject | undefined {
   ...
   * @returns {string} 收件人邮箱地址，多个收件人用逗号分隔
   */
  get recipient(): string {
    return this.recipientEmails.join(',');
  }
   ...
   * @param subject 邮件主题（可选）
   * @param metadata 元数据
   * @returns {EmailNotification} 邮件通知实例
   */
  static create(
    tenantId: Uuid,
    templateId: Uuid,
   ...
   * @description 从字符串创建邮件通知实例
   * @param tenantId 租户ID
   * @param templateId 模板ID
   * @param recipientEmails 收件人邮箱地址字符串列表
   * @param data 模板数据
   * @param priority 优先级
   * @param scheduledAt 计划发送时间
   * @param subject 邮件主题字符串（可选）
   * @param metadata 元数据
   * @returns {EmailNotification} 邮件通知实例
   */
  static createFromStrings(
    ...
    // 验证收件人列表不能为空
    if (emails.length === 0) {
      throw new Error('邮件通知必须包含至少一个收件人');
    }
